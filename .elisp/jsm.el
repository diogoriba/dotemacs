...
(defconst js--font-lock-keywords-1
  ...
  "Level one font lock keywords for `jsm-mode'.")
...
(defconst js--font-lock-keywords-2
  ...
  "Level two font lock keywords for `jsm-mode'.")
...
(defcustom js-indent-level 4
  "Number of spaces for each indentation step in `jsm-mode'."
  :type 'integer
  :group 'js)
...
(defcustom js-js-switch-tabs
  ...
  "Whether `jsm-mode' should display tabs while selecting them.
...
(defcustom js-js-tmpdir
  "~/.emacs.d/js/js"
  "Temporary directory used by `jsm-mode' to communicate with Mozilla.
This directory must be readable and writable by both Mozilla and Emacs."
  :type 'directory
  :group 'js)

(defcustom js-js-timeout 5
  "Reply timeout for executing commands in Mozilla via `jsm-mode'.
...
(defvar jsm-mode-syntax-table
  (let ((table (make-syntax-table)))
    (c-populate-syntax-table table)
    (modify-syntax-entry ?$ "_" table)
    table)
  "Syntax table for `jsm-mode'.")

(defvar js--quick-match-re nil
  "Autogenerated regexp used by `jsm-mode' to match buffer constructs.")

(defvar js--quick-match-re-func nil
  "Autogenerated regexp used by `jsm-mode' to match constructs and functions.")

(make-variable-buffer-local 'js--quick-match-re)
(make-variable-buffer-local 'js--quick-match-re-func)

(defvar js--cache-end 1
  "Last valid buffer position for the `jsm-mode' function cache.")
...
(defun js--update-quick-match-re ()
  "Internal function used by `jsm-mode' for caching buffer constructs.
...
(defun js-beginning-of-defun (&optional arg)
  "Value of `beginning-of-defun-function' for `jsm-mode'."
  (setq arg (or arg 1))
...
(defun js--flush-caches (&optional beg ignored)
  "Flush the `jsm-mode' syntax cache after position BEG.
...
(defun js-end-of-defun (&optional arg)
  "Value of `end-of-defun-function' for `jsm-mode'."
  (setq arg (or arg 1))
...
(defun js--backward-syntactic-ws (&optional lim)
  "Simple implementation of `c-backward-syntactic-ws' for `jsm-mode'."
  (save-restriction
...
(defun js--forward-syntactic-ws (&optional lim)
  "Simple implementation of `c-forward-syntactic-ws' for `jsm-mode'."
  (save-restriction
  ...
  `(
    ...
    ,(list
      ...
      (list js--name-re
            ...
            '(end-of-line)
            '(0 font-lock-variable-name-face))))
  "Level three font lock for `jsm-mode'.")
...
(defun js--class-decl-matcher (limit)
  "Font lock function used by `jsm-mode'.
...
(defconst js--font-lock-keywords
  ...
  "Font lock keywords for `jsm-mode'.  See `font-lock-keywords'.")
...
This searches all existing `jsm-mode' buffers. Each key is the
...
;;;###autoload
(define-derived-mode jsm-mode nil "js"
...
\\{jsm-mode-map}"
  ...
  :group 'js
  :syntax-table jsm-mode-syntax-table
  ...
  (setq major-mode 'jsm-mode)
  (setq mode-name "Javascript-m")
...
;;;###autoload
(defalias 'javascript-mode 'jsm-mode)

(eval-after-load 'folding
  '(when (fboundp 'folding-add-to-marks-list)
     (folding-add-to-marks-list 'jsm-mode "// {{{" "// }}}" )))
